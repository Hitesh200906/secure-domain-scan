      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link
              to="/dashboard"
              className="px-4 py-2 text-[13px] text-muted-foreground hover:text-white transition"
            >
              Login
            </Link>
            <Link
              to="/contact"
              className="relative inline-flex items-center gap-1.5 rounded-full bg-white text-black px-4 py-2 text-[13px] font-medium hover:bg-primary transition-all duration-300"
            >
              Start Free Scan
          ...
          <div className="md:hidden mt-2 glass-strong rounded-2xl p-4 space-y-1">
            {links.map((l) => (
              <Link
                ...
                className="block px-3 py-2.5 text-sm text-muted-foreground hover:text-white rounded-lg"
              >
                {l.label}
              </Link>
            ))}
            <div className="pt-2 mt-2 border-t border-white/10 flex flex-col gap-2">
              <Link
                to="/dashboard"
                onClick={() => setOpen(false)}
                className="px-3 py-2.5 text-sm text-muted-foreground hover:text-white rounded-lg"
              >
                Login
              </Link>
              <Link
                to="/contact"
                onClick={() => setOpen(false)}
                className="text-center rounded-full bg-white text-black px-4 py-2.5 text-sm font-medium"
              >
                Start Free Scan →
              </Link>
            </div>
