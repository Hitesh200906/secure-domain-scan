...
export const Route = createFileRoute("/business/store/edit")({ component: StoreEdit });
...
function StoreEdit() {
  ...
  const [f, setF] = useState({
    ...
    theme_color: store.theme_color ?? "#7c3aed",
    accent_color: store.accent_color ?? "#22d3ee",
  });
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <Link to="/business/store" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ChevronLeft className="size-4" /> Back to store
      </Link>
      <div>
        <h1 className="text-2xl font-semibold">Edit store</h1>
        <p className="text-sm text-muted-foreground mt-1">Update your public storefront details.</p>
      </div>
